...
type CouncilState = "loading" | "none" | "pending" | "approved" | "rejected";

export default function ProfilePage() {
  const [user, setUser] = useState<User | null>(null);
  const [loading, setLoading] = useState(true);
  const [councilStatus, setCouncilStatus] = useState<CouncilState>("loading");
      ...
      getCouncilStatus(currentUser.id, token)
        .then((member) => {
          if (member?.status) {
            ...
            // Also fallback check if user is approved (in case member lookup failed)
  ...
  async function handleApplyCouncil() {
    ...
    try {
      ...
      setApplyMessage("Your application has been submitted successfully! Existing council members can now review and approve your request.");
    } catch (err: any) {
    ...
    <main className="min-h-screen bg-[#FBF7EC] px-4 py-8 text-[#1A1A1A] sm:px-8">
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <header className="flex items-center justify-between border-b border-[#1A1A1A]/10 pb-5">
          ...
          <div className="flex items-center gap-3">
            {isCouncil && (
              <Link
                href="/council"
                className="rounded-full border border-[#B8860B] px-5 py-2.5 text-xs font-semibold text-[#B8860B] transition hover:bg-[#B8860B] hover:text-white"
              >
                ⚖️ Council
              </Link>
            )}
        ...
        <section className="py-12 sm:py-16">
            ...
            <div className="mt-6 rounded-[2rem] border border-[#B8860B]/30 bg-[#B8860B]/[0.08] p-7 sm:p-9">
              <div className="flex flex-col gap-7 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-5">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold">Council Dashboard</h3>
                    <p className="mt-2 max-w-xl text-sm leading-6 text-[#1A1A1A]/55">
                      You are an approved HerAI Council member. Review pending membership applications and configure regional AI domain rules.
                    </p>
                  </div>
                </div>
                <Link
            ...
            <div className="mt-6 rounded-[2rem] border border-amber-200 bg-amber-50/60 p-7 sm:p-9">
              <div className="flex flex-col gap-7 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-5">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-[#1A1A1A]">Council Application Pending</h3>
                    <p className="mt-2 max-w-xl text-sm leading-6 text-[#1A1A1A]/65">
                      Your application to join the HerAI Council has been submitted. Existing council members can review and approve your request from the council dashboard.
                    </p>
                  </div>
                </div>
          ...
          {(councilStatus === "none" || councilStatus === "rejected") && (
            ...
            <div className="mt-6 rounded-[2rem] border border-[#1A1A1A]/10 bg-white p-7 shadow-xl shadow-[#1A1A1A]/5 sm:p-9">
              <div className="flex flex-col gap-7 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-5">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold">
                      {councilStatus === "rejected" ? "Council Application" : "Join the HerAI Council"}
                    </h3>
                    <p className="mt-2 max-w-xl text-sm leading-6 text-[#1A1A1A]/55">
                ...
                <button
                  ...
                  className="inline-flex min-h-11 shrink-0 items-center justify-center rounded-full bg-[#1A1A1A] px-7 text-sm font-semibold text-white transition hover:bg-[#333] disabled:opacity-60"
                >
                  {applying ? "Submitting..." : councilStatus === "rejected" ? "Re-apply for Council" : "Apply for Council →"}
                </button>
              </div>
            </div>
          )}
